...
public class RegisterController {
	...
	//개인 회원 가입 페이지 접근
	@RequestMapping(value={"/common/privateRegister.do","/smart/s_privateRegister.do"}, method=RequestMethod.GET)
	public String privateRegister(HttpServletRequest request){
		...
		if(request.getRequestURI().equals("/smart/s_privateRegister.do")){
			url= "smart.s_register.s_privateRegister";
		}else{
			url= "register.privateRegister";
		}
	...
	//기업 회원 가입 input
	@Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor=Exception.class)
	@RequestMapping(value={"/common/comRegister.do","/smart/s_comRegister.do"}, method=RequestMethod.POST)
	public String comRegisterOk(HttpServletRequest request, Model mv, RegisterMemberDto rdto){
		...
		try{
			...
			if(defaultCheck!=null){
				if(defaultCheck.getAlive().equals("1")){
					...
					if(request.getRequestURI().equals("/smart/s_comRegister.do")){
