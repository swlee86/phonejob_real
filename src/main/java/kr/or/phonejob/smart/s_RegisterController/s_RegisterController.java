package kr.or.phonejob.smart.s_RegisterController;
...
import kr.or.phonejob.smart.s_Service.s_LoginService;
import kr.or.phonejob.smart.s_Service.s_RegisterService;
...
@Controller
public class s_RegisterController {

	private static final Logger logger = LoggerFactory.getLogger(s_RegisterController.class);
	
	//RegisterService
	@Autowired
	private s_RegisterService rservice;
	
	
	@Autowired
	private s_LoginService lservice;
	...
	//개인 회원 가입 페이지 접근
	@RequestMapping(value="/smart/s_privateRegister.do", method=RequestMethod.GET)
	public String privateRegister(HttpServletRequest request){
		String url ="smart.s_register.s_privateRegister";
	...
	@RequestMapping(value="/smart/s_privateRegister.do", method=RequestMethod.POST)
	public String privateRegisterOk(Model mv, s_RegisterMemberDto rdto){
		...
		try{
			...
			s_MemberDetailDto defaultCheck = rservice.privateRegisterSelect(rdto.getUserid());
			...
			if(defaultCheck!=null){
				if(defaultCheck.getAlive().equals("1")){
					...
					movepage="../smart/s_privateRegister.do";
					page="smart.s_register._registerRedirect";
				}
			}else{
				logger.info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"+rdto.toString());
			...
			s_MemberDetailDto pjdto = rservice.privateRegisterSelect(rdto.getUserid());
			...
			s_RegisterGradeDto rgdto = new s_RegisterGradeDto();
			...
			if(result1==1 && result2==1 &&result3==1 && result4==1){
				data="회원가입에 성공하였습니다.";
				movepage="../s_Main.do";
				page="smart.s_register.s_registerRedirect";
			}else{
	...
	//기업회원 가입 페이지 접근
	@RequestMapping(value="/smart/s_comRegister.do", method=RequestMethod.GET)
	public String comRegister(){
		String url ="smart.s_register.s_comRegister";
		try{
	...
	//기업 회원 가입 input
	@Transactional(propagation = Propagation.REQUIRES_NEW, rollbackFor=Exception.class)
	@RequestMapping(value="/smart/s_comRegister.do", method=RequestMethod.POST)
	public String comRegisterOk(Model mv, s_RegisterMemberDto rdto){
		...
		try{
			...
			if(defaultCheck!=null){
				if(defaultCheck.getAlive().equals("1")){
					logger.info("이미 가입 되어 있는 경우");
					data="이미 가입되어 있는 아이디입니다.";
					movepage="../smart/s_comRegister.do";
					page="smart.s_register.s_registerRedirect";
				}
			}else{
				logger.info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"+rdto.toString());
			...
			if(result1==1 && result2==1 &&result3==1 && result4==1 && result5==1){
				data="회원가입에 성공하였습니다.";
				movepage="../s_Main.do";
				page="snart.s_register.s_registerRedirect";
			}else{
				data="회원가입에 실패하였습니다. 잠시 후 다시 시도해 주세요";
				movepage="../smart/s_comRegister.do";
